...
export class ModuleApi {
   ...
   private url = `${ENV_API_URL}/v1/modules`;
   ...
   async create(pData: CreateModuleReq): Promise<AxiosResponse> {
      try {
         const config: AxiosRequestConfig<string> = {
            headers: {
               Authorization: `Bearer ${this.bearer}`,
   ...
   async update(pId: string, pData: UpdateModuleReq): Promise<AxiosResponse> {
      try {
         const config: AxiosRequestConfig<string> = {
            headers: {
               Authorization: `Bearer ${this.bearer}`,
